...
import { contactsService, labelsService } from '@/services/contacts';
import {
  Contact,
  ContactsState,
  ContactsListParams,
...
export default function Contacts() {
  ...
  const getSelectedLabels = useCallback(
    (labelsOverride?: string[]) => {
      const labels = labelsOverride ?? selectedTagFilters;
  ...
  const loadContacts = useCallback(
    async (params?: Partial<ContactsListParams>, labelsOverride?: string[]) => {
      ...
      try {
        const requestParams: ContactsListParams = {
          ...
          order: 'asc',
          ...params,
          labels: getSelectedLabels(labelsOverride),
        };
  ...
  const loadContactsWithSearch = useCallback(
    async (
      ...
      try {
        const searchParams: ContactsSearchParams = {
          q: query,
          page: params?.page || 1,
  ...
  useEffect(() => {
    ...
    loadContacts();
    loadAvailableLabels();
  }, [permissionsReady, loadContacts, loadAvailableLabels]);
  ...
  const handleApplyFilters = async (filters: BaseFilter[]) => {
    ...
    try {
      ...
      const response = await contactsService.filterContacts({
        ...
        labels: selectedTagFilters.length > 0 ? selectedTagFilters : undefined,
      });
  ...
  const handleApplyFiltersWithPagination = async (
    ...
    try {
      ...
      const response = await contactsService.filterContacts({
        page,
        payload: filterPayload,
        labels: (labelsOverride ?? selectedTagFilters).length > 0
          ? (labelsOverride ?? selectedTagFilters)
          : undefined,
      });
  ...
  const handleTagFilterChange = async (labels: string[]) => {
    setSelectedTagFilters(labels);
    setState(prev => ({
      ...prev,
      meta: {
        ...prev.meta,
    ...
    if (state.searchQuery.trim()) {
      await loadContactsWithSearch(
        state.searchQuery.trim(),
        { page: 1, per_page: state.meta.pagination.page_size },
        labels,
      );
      return;
    }
    ...
    if (activeFilters.length > 0) {
      await handleApplyFiltersWithPagination(
        activeFilters,
        1,
        state.meta.pagination.page_size,
        labels,
      );
      return;
    }
    ...
    await loadContacts(
      {
        ...
        sort: state.sortBy as ContactsListParams['sort'],
  ...
  const tagAppliedFilters: AppliedFilter[] = selectedTagFilters.map(label => ({
    id: `tag-${label}`,
    label: t('tagFilter.appliedLabel'),
    value: label,
    onRemove: () => {
      void handleTagFilterChange(selectedTagFilters.filter(current => current !== label));
    },
  }));

  const headerFilters = [...appliedFilters, ...tagAppliedFilters];
