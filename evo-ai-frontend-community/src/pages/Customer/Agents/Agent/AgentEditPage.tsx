...
const AgentEditPage = () => {
  ...
  useEffect(() => {
    ...
    if (activeMenu === 'tools') {
      if (agentTools.length > 0 && agentToolsData.length === 0) {
        void loadAgentToolsData(agentTools)
          .then(data => {
            setAgentToolsData(data as unknown as Agent[]);
          })
          .catch(() => {});
      }
      return;
    ...
    if (activeMenu === 'integrations' && Object.keys(integrations).length === 0) {
      void getAgentIntegrations(id)
        .then(backendIntegrations => {
          const mergedIntegrations: Record<string, any> = {
